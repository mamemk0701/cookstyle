      ...
      class OutdatedPackageCheck < Base
        ...
        def on_block(node)
          ...
          if package_version == 'latest'
            add_offense(node, message: "#{package_name} version is 'latest'. Security Smell: False Positive (Correct, but may trigger outdated check).")
            return
          end
          ...
          if LATEST_VERSIONS.key?(package_name) && version_outdated?(package_version, LATEST_VERSIONS[package_name])
            add_offense(node, message: "#{package_name} version #{package_version} is outdated. Latest: #{LATEST_VERSIONS[package_name]}.")
          end
        end
